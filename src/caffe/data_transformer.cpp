...
template<typename Dtype>
cv::Mat DataTransformer<Dtype>::DatumToCVMat(const Datum& datum){
...
template<typename Dtype>
cv::Mat DataTransformer<Dtype>::DatumLabelToCVMat(const Datum& datum){
int datum_height = datum.height();
int datum_width = datum.width();

cv::Mat cv_img;

    cv_img.create(datum_height, datum_width, CV_8UC1);
    const string& data = datum.labeldata();
    std::vector<char> vec_data(data.c_str(), data.c_str() + data.size());
...
template<typename Dtype>
void DataTransformer<Dtype>::random_darker(cv::Mat& img)
{
  float rand = ((float)(6+Rand(5)))/10.0;
  int h = img.rows;
  int w = img.cols;
  int c = img.channels();
  ...
  for(int i=0;i<h*w*c;i++)
  {
    ...
    imgData[i] = static_cast<uchar>(pData);
  }
}


template<typename Dtype>
void DataTransformer<Dtype>::random_brighter(cv::Mat& img)
{


  float rand = ((float)(10+Rand(11)))/10.0;
  int h = img.rows;
  int w = img.cols;
  int c = img.channels();
  unsigned char* imgData = img.data;
...
template<typename Dtype>
void DataTransformer<Dtype>::random_guass(cv::Mat& img)
{
  int kernel_size = Rand(6)*2+1;
  cv::GaussianBlur(img,img,cv::Size(kernel_size,kernel_size),0,0);
}
...
template<typename Dtype>
void DataTransformer<Dtype>::random_addGaussianNoise(cv::Mat& img)
{
  float rand = ((float)Rand(3))/10.0;
  int w = img.cols;
  int h = img.rows;
  int g_noisenum = (int)(rand*(float)w*(float)h);
  for(int i=0;i<g_noisenum;i++)
  {
    int temp_x = Rand(h);
    int temp_y = Rand(w);
    int temp_z = Rand(3);
    int val = img.at<cv::Vec3b>(temp_x, temp_y)[temp_z] + generateGaussianNoise(2, 0.8) * 32;
    if(val >= 255){val = 255;}
    else if(val<=0){val =0;}
    else{}
    
    img.at<cv::Vec3b>(temp_x, temp_y)[temp_z] = static_cast<uchar>(val);
    
  }
}

template<typename Dtype>
void DataTransformer<Dtype>::random_saltAndPepper(cv::Mat& img)
{
  float rand = ((float)Rand(3))/10.0;
  int w = img.cols;
  int h = img.rows;
  int sp_noisenum = (int)(rand*(float)w*(float)h);
  for(int i=0;i<sp_noisenum;i++)
  {
    ...
    if(k)
    {
      img.at<cv::Vec3b>(temp_x, temp_y)[temp_z]= 0;
    }
    else
    {
      img.at<cv::Vec3b>(temp_x, temp_y)[temp_z]=255;
    }
...
template<typename Dtype>
void DataTransformer<Dtype>::random_rotate(cv::Mat& img,vector<float>& currentLabels,int n_point)
{
  float h = (float)img.rows;
  float w = (float)img.cols;
  float cx = w/2;
  float cy = h/2;
  int plusminus = Rand(10);
  int cx_delta = Rand(15);
  int cy_delta = Rand(15);
  if(plusminus>4)
  {
    cx += cx_delta;
    cy += cy_delta;
  ...
  double angle = (double)(Rand(41)-20);
  double scale = ((double)(Rand(4)+7))/10.0;
  
  cv::Mat m = cv::getRotationMatrix2D(cv::Point2f(cx,cy),angle,scale);
  cv::warpAffine(img,img,m,cv::Size(w,h));

  for(int i=0;i<n_point;i++)
  {
    float ld_x = currentLabels[2*i];
    float ld_y = currentLabels[2*i+1];
    float ld_x_t = (float)m.at<double>(0,0)*ld_x*w+(float)m.at<double>(0,1)*ld_y*h+(float)m.at<double>(0,2);
    float ld_y_t = (float)m.at<double>(1,0)*ld_x*w+(float)m.at<double>(1,1)*ld_y*h+(float)m.at<double>(1,2);
    currentLabels[2*i] = ld_x_t/w;
    currentLabels[2*i+1] = ld_y_t/h;
...
template<typename Dtype>
void DataTransformer<Dtype>::faceTransform(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob) {
  const int img_channels = cv_img.channels();
...
template<typename Dtype>
void DataTransformer<Dtype>::segLabelTransform(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob,Dtype* label) {
//   const int img_channels = cv_img.channels();
  const int img_height = cv_img.rows;
  const int img_width = cv_img.cols;
...
template<typename Dtype>
void DataTransformer<Dtype>::ldmarkTransform(const Datum& datum, Blob<Dtype>* transformed_blob,
					     vector<float>& currentLabels,int label_size)
{
  ...
  {
      currentLabels.push_back(datum.labels(i));
  }      
  ...
  if(rand1<3)
  {
    random_brighter(cv_img);
  }
  else if(rand1>2 && rand1<6)
  {
    random_darker(cv_img);
  }
  
  int rand2 = Rand(9);
  if(rand2<3)
  {
    random_addGaussianNoise(cv_img);
  }
  else if(rand2>2 && rand2<6)
  {
    random_saltAndPepper(cv_img);
  }
  
  int rand3 = Rand(2);
  if(rand3) { random_guass(cv_img); }
   
  int rand4 = Rand(2);
  if(rand4) { random_rotate(cv_img,currentLabels,label_size/2);}
  
  faceTransform(cv_img,transformed_blob);
  
//   int h = cv_img.rows;
//   int w = cv_img.cols;
//   for(int i=0;i<label_size/2;i++)
//   {
//     int x = (int)((float)w*currentLabels[2*i]);
//     int y = (int)((float)h*currentLabels[2*i+1]);
//     std::cout<< x <<" "<<y<<std::endl;
//     cv::circle(cv_img,cv::Point(x,y),2,cv::Scalar(0,0,255),-1);
//     
//   }
...
template<typename Dtype>
void DataTransformer<Dtype>::segDataTransform(const Datum& datum, Blob<Dtype>* data_blob,Dtype* label)
{
  cv::Mat cv_img = DatumToCVMat(datum);
  cv::Mat label_img = DatumLabelToCVMat(datum);
  
  faceTransform(cv_img,data_blob);
  segLabelTransform(label_img,data_blob,label);
