...
bool ReadImageToDatum(const string& filename, const string& labelfilename,
    const int height, const int width, const bool is_color,
    const std::string & encoding, Datum* datum)
{
  
      cv::Mat cv_img = ReadImageToCVMat(filename, height, width, is_color);
      cv::Mat label_img = ReadImageToCVMat(labelfilename, height, width, false);
      
      if (cv_img.data && label_img.data) {

        CVMatToDatum(cv_img,label_img,datum);
...
void CVMatToDatum(const cv::Mat& cv_img, const cv::Mat& label_img, Datum* datum)
{
  CHECK(cv_img.depth() == CV_8U) << "Image data type must be unsigned byte";
  CHECK(label_img.depth() == CV_8U) << "Image data type must be unsigned byte";
  datum->set_channels(cv_img.channels());
  datum->set_height(cv_img.rows);
  datum->set_width(cv_img.cols);
  datum->clear_data();
  datum->clear_labeldata();
  datum->clear_float_data();
  datum->set_encoded(false);
  int datum_channels = datum->channels();
  int datum_height = datum->height();
  int datum_width = datum->width();
  int datum_size = datum_channels * datum_height * datum_width;
  int label_size = datum_height * datum_width;
  std::string buffer(datum_size, ' ');
  std::string label_buffer(label_size, ' ');
  ...
  for (int h = 0; h < datum_height; ++h) {
    const uchar* labelptr = label_img.ptr<uchar>(h);
    int label_index = 0;
    for (int w = 0; w < datum_width; ++w) {
        int datum_index = h * datum_width + w;
        label_buffer[datum_index] = static_cast<char>(labelptr[label_index++]);
    }
  }
  
  datum->set_data(buffer);
  datum->set_labeldata(label_buffer);
