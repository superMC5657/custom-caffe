...
void MultilabelDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->output_labels_){
      ...
      label_shape[1] = this->layer_param_.multilabel_data_param().num_point()*2;
...
void MultilabelDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int label_size = this->layer_param_.multilabel_data_param().num_point()*2;
      
    this->data_transformer_->ldmarkTransform(datum,&(this->transformed_data_),current_label,label_size);
//       this->data_transformer_->Transform(datum, &(this->transformed_data_));

      for (int i = 0; i < label_size; i++)
      {
          top_label[item_id * label_size + i] = current_label[i];
      }      
