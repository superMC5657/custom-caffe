...
class DataTransformer {
 public:
  ...
  void faceTransform(const cv::Mat& cv_img,Blob<Dtype>* transformed_blob);
  void ldmarkTransform(const Datum& datum, Blob<Dtype>* transformed_blob,
		       vector<float>& currentLabels,int label_size);
  void random_brighter(cv::Mat& img);
  void random_darker(cv::Mat& img);
  void random_guass(cv::Mat& img);
  void random_saltAndPepper(cv::Mat& img);
  void random_addGaussianNoise(cv::Mat& img);
  void random_rotate(cv::Mat& img,vector<float>& currentLabels,int n_point);
  cv::Mat DatumToCVMat(const Datum& datum);
  cv::Mat DatumLabelToCVMat(const Datum& datum);
  void segLabelTransform(const cv::Mat& cv_img,Blob<Dtype>* transformed_blob,Dtype* label);
   
  void segDataTransform(const Datum& datum, Blob<Dtype>* data_blob,Dtype* label);
  /**
   * @brief Applies the transformation defined in the data layer's
